...
struct
	...
	let empty =
		Lwt_unix.run
			(Store.create config task >>= fun v -> return (v "??"))
	...
	let to_list t =
		Lwt_unix.run
			((View.of_path t [ "Tuples" ]) >>= (fun v -> TupleView.t_of_view v))
...
struct
	...
	let map g ~tuples: (query) =
		((IS.query g query) |> List.flatten) |> IS.init
	
	(* Helper.flatten_tuple_list |> IS.init *)
