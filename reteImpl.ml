...
(* CONNECTION WITH THE USE OR PERFORMANCE OF THIS SOFTWARE. ------ RETE    *)
(* ------------                                                            *)
...
module InMemory = struct
	...
	type am =
		{
			tuples : tuple list; (* for convinience - direct access to tuples without going through variables. *)
			pattern : tuple;
