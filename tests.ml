(* * Copyright (c) 2015 Yan Shvartzshnaider * * Permission to use, copy,   *)
...
(* TEST 0: MAP { ?x, type, Car ?x, hasColor, Red } *)
let test0 _ =
...
(* print_tuples_list res_q1;; print_tuples_list q1_exp_res;; TEST 1: MAP { *)
(* ?x, type, Car ?x, color, ?y ?y, type, Color ?y, rgbValue, White }test1  *)
let test1 _ =
  let query = [ q1; q3; q4; q5 ] and q_exp_res = [ [ t1; t2; t5; t7 ] ] in
  let res_q = execute_query tuples query in assert_equal q_exp_res res_q

(* TEST 2: similar to test 1 query but the order of the tuple template is  *)
(* different MAP { ?y, rgbValue, White ?x, type, Car ?x, color, ?y ?y,     *)
(* type, Color }                                                           *)
let test2 _ =
  let query = [ q5; q1; q3; q4 ] and q_exp_res = [ [ t7; t1; t2; t5 ] ] in
  let res_q = execute_query tuples query in assert_equal q_exp_res res_q

(* print_tuples_list q2_exp_res;; TEST 3: - Note - expected results are    *)
(* order sensetive MAP { ?x, relatesTo, ?y ?y, relatesTo, ?x }             *)
let test3 _ =
...
*)
(* TODO TEST : MAP { ?X type GraduateStudent, ?X takesCourse               *)
(* GraduateCourse0} TODO TEST : MAP { X type GraduateStudent, ?Y type      *)
(* University, ?Z type Department, ?X memberOf ?Z, ?Z subOrganizationOf    *)
(* ?Y, ?X undergraduateDegreeFrom ?Y } TODO TEST : MAP { ?X type Student,  *)
(* ?Y type Faculty, ?Z type Course, ?X advisor ?Y, ?Y teacherOf ?Z, ?X     *)
(* takesCourse ?Z, } test filter function                                  *)
let test4 _ =
...
(* TEST : Implement by create a combination of AM and BM MAP { ?x, type,   *)
(* Car ?x, hasColor, Red }                                                 *)
let test7 _ =
  let module Test =
  struct
...
(* TEST 8: MAP { ?x, type, Car ?x, hasColor, ?y ?y, type, Color ?y,        *)
(* rgbValue, White }                                                       *)
let test8 _ =
  let module Test =
  struct
...
(* TEST 9: MAP { ?x, type, Car ?x, hasColor, ?y ?y, type, Color ?y,        *)
(* rgbValue, White }                                                       *)
let test9 _ =
  let module Test =
  struct
...
(* Tests for Irmin backed TEST 11: MAP { ?x, type, Car ?x, hasColor, Red } *)
let test11 _ =
  let db = Moana_irmin.S.init tuples in
  let query10 = [ q1; q2 ] and q10_exp_res = [ [ t1; t9 ] ] in
  let res_q10 = Moana_irmin.S.query db query10
  in
  (* let p = print_tuples_list res_q10 in let p1 = print_tuples_list     *)
  (* q10_exp_resin                                                       *)
  assert_equal q10_exp_res res_q10

(* Test 12 Tests the add function and to_list function *)
let test12 _ =
...
(* TEST 16: Convert query string to list of query tuples MAP { ?x, type,   *)
(* Car ?x, hasColor, Red }                                                 *)
let test16 _ =
  let res =
    str_query_list
...
Note, this test failed when tabs were converted to spaces.     
*)
let test17 _ =
  let res =
    to_tuple_lst
...
let test21 _ =
  ...
  struct
    ...
    (* let am1 = create_am q1 [] and am2 = create_am q3 [] and am3 = create_am *)
    (* q4 [] and am4 = create_am q5 [] let am_list = [ am1; am2; am3; am4 ]    *)
    (* let p = print_mappings am2 let rete_network = gen_rete am_list          *)
  ...
  in
  (* let p = ReteImpl.InMemory.node_to_json Test.res_node |>                 *)
  (* Rete_node_j.string_of_node_json |> Yojson.Basic.from_string             *)
  (* |>Yojson.Basic.pretty_to_string |> print_string in                      *)
  assert_equal Test.res_bm Test.q_exp_res
...
(*** TEST 24 - NOT COMPLETE:

Tests add tuple function in Rete / Irmin
...
let suite =
  ...
  "test21:" >:: test21; "test22:" >:: test22; "test23:" >:: test23; "test24:" >:: test24  ]
